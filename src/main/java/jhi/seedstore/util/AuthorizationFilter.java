...
public class AuthorizationFilter implements ContainerRequestFilter
{
		...
		throws IOException
	{
		...
		List<UsersUserType> classRoles = extractRoles(resourceClass);

		// Get the resource method which matches with the requested URL
		// Extract the roles declared by it
		Method resourceMethod = resourceInfo.getResourceMethod();
		List<UsersUserType> methodRoles = extractRoles(resourceMethod);
	...
	private List<UsersUserType> extractRoles(AnnotatedElement annotatedElement)
	{
		...
		else
		{
			...
			{
				UsersUserType[] allowedRoles = secured.value();
				return Arrays.asList(allowedRoles);
			}
		}
	}

	private void checkPermissions(List<UsersUserType> allowedRoles, AuthenticationFilter.UserDetails userDetails)
		throws Exception
	{
		if (userDetails != null)
		{
